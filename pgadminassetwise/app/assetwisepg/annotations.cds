using assetwise_pg as service from '../../srv/service';
...
annotate service.AssetMaster with @(
    ...
    UI.FieldGroup #GeneratedGroup6 : {
        ...
        Data : [
            ...
            {
                $Type : 'UI.DataField',
                Label : '{i18n>ActualScrappedDate}',
                Value : assetActScrappedDate,
                ![@Common.FieldControl]: { $edmJson : {$If : [ { $Eq : [{$Or:[ { $Path : 'breakdown'},{ $Path : 'lost'}]}, false ]}, 3, 1 ]}},
            },
            {
                $Type : 'UI.DataField',
    ...
    user @Common.ValueList : {
        ...
        Parameters : [
            ...
            },
            {
                ...
                LocalDataProperty: userTeam,
                ValueListProperty: 'team'
            }
        ],
...
annotate service.AssetMaster with {
    ...
    ownedTeam @mandatory : true;
    Owner @mandatory : true;
    userTeam @mandatory : true;
    user @mandatory : true;
    // new fixes 
    // nextRenewalDate @assert.range: [{min: 'previousRenewalDate'}];
    // previousRenewalDate @assert.range: [{max: 'nextRenewalDate'}];

    // returnDate @assert.range: [{min: 'handedOverDate'}];
    // handedOverDate @assert.range: [{max: 'returnDate'}];

    matType @Common.ValueListForValidation : 'MaterialType';
    Owner @Common.ValueListForValidation : 'Employee';
    user @Common.ValueListForValidation : 'Employee';
    assetScrappedOwner @Common.ValueListForValidation : 'Employee';
    // assetScrappedValue @assert.format: '^[+]?[(]?[0-9]{1,4}[)]?[-\\s\\.]?[0-9]{1,4}[-\\s\\.]?[0-9]{1,9}(\\.[0-9]+)?$';
    // assetScrappedValue @assert.format.errorMessage: 'The Asset Scrapped Value must be a valid number (e.g., positive integers or decimals).';
};
annotate service.AssetMaster with {
