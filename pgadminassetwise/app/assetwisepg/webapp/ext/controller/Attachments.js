	...
	function (Controller, MessageToast) {
		...
		return {
			...
			handleValueChange: function (oEvent) {
				...
				// Condition to check file size (60KB = 60 * 1024 = 61,440 bytes)
				if (item.size > 61440) {
					sap.m.MessageToast.show("File size exceeds 60KB. Please upload a smaller file.");
			...
			_createEntity: function (item, index) {
				...
				if (item && item.mediaType) {
					if (item.mediaType.includes("pdf")) {
						item.url = item.url || "sap-icon://pdf-attachment";
					} else if (item.mediaType.includes("image")) {
						item.url = item.url || "sap-icon://attachment-photo";
					} else if (item.mediaType.includes("text")) {
						item.url = item.url || "sap-icon://document-text";
					} else {
						item.url = item.url || "sap-icon://document"; // Default icon
					}
				}
				return new Promise((resolve, reject) => {
