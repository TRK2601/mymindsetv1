sap.ui.define([
    ...
    function (Controller, JSONModel, formatter, Fragment, Filter, MessageToast, Dialog, Button, Label, mobileLibrary, Text, TextArea, MessageBox) {
        "use strict";
        var oController, oGlobalModel, sViewBindingCtxPath, sServiceURL, oDataModel;
        // shortcut for sap.m.ButtonType
	    var ButtonType = mobileLibrary.ButtonType;
        // shortcut for sap.m.DialogType
	    var DialogType = mobileLibrary.DialogType;
        return Controller.extend("com.mindset.ui.assetwisepg.workflow.controller.Detail", {
            formatter: formatter,
            onInit: function () {
                oController = this;
            ...
            _onPatternMatch:function(oEvent){
                ...
                if(sID){
                    jQuery.ajax({
                        url: sServiceURL + "/APRequests/"+sID +"?$expand=lineitems,comments",
                        async: false,
                        dataType: "json",
            ...
            onPressSubmit: function(){
                ...
                let sAssignID = oViewBindingObj && oViewBindingObj.assigneeID;
                let sManagerID = oGlobalModel.getProperty("/LoginEmployeeData") && oGlobalModel.getProperty("/LoginEmployeeData")[0] && oGlobalModel.getProperty("/LoginEmployeeData")[0].managerEmail;
                if (!sAssignID && !oViewBindingObj.ID) {
                    MessageToast.show("Please select Assignee ID..!");
                    oGlobalModel.setProperty("/sAssigneeIDValueStateReq", "Error"); //TBD
                } else if (!sManagerID && !oViewBindingObj.ID) {
                    MessageToast.show("Please select Manager ID..!");
                } else {
            ...
            _showCommentDlg: function(sRole, sAction){
                var oModel = this.getView().getModel("GlobalModel");
                //TBD
                oGlobalModel.setProperty("/sApproverNoteCommentReq", "");
                oGlobalModel.setProperty("/sApproverNoteCommentValueReq", "None");
                ...
                this.oCmtDialog = new Dialog({
                    ...
                    content: [
                        ...
                        new TextArea("submissionNote", {
                            ...
                            width: "100%",
                            valueState: oGlobalModel.getProperty("/sApproverNoteCommentValueReq") || "None",
                            ...
                            change: function(oEvent){
                                var sText = oEvent.getParameter("value") || "";
                                oGlobalModel.setProperty("/sApproverNoteCommentReq", sText); //TBD
                                oGlobalModel.setProperty("/sApproverNoteCommentValueReq", sText ? "None" : "Error");//TBD
                                aComments.push({
                    ...
                    buttons: [
                        new Button({
                            ...
                            press: function () {
                                if (bIsApprover && !oGlobalModel.getProperty("/sApproverNoteCommentReq")) {
                                    oGlobalModel.setProperty("/sApproverNoteCommentValueReq", "Error");
                                } else {
            ...
            onAssignToVHelpReqPress:function(oEvent){
                ...
                oModel.setProperty("/sAssigneeIDValueStateReq", "None");
                if (!this._pValueHelpDialog) {
                    this._pValueHelpDialog = Fragment.load({
                        id: oView.getId(),
                        name: "com.mindset.ui.assetwisepg.workflow.fragments.Dialogs.AssignToValueHelp",
                        controller: this
            ...
            handleValueHelpClose: function (oEvent) {
                ...
                var oModel = oController.getOwnerComponent().getModel("GlobalModel");
                let oEmpData = oModel.getProperty("/LoginEmployeeData") || [];
                if (oEmpData && oEmpData.length > 0 && oEmpData[0].managerEmail &&  oEmpData[0].managerEmail.toLowerCase() !== oBindingObj.email.toLowerCase()) {
                    ...
                    // TBD
                    let sText = "";
                    oModel.setProperty("/sAssigneeIDValueStateReq", "Error");
                    if (!oEmpData || oEmpData.length === 0 || !oEmpData[0].managerEmail) {
                        sText = "Manager ID is empty, please check Master Data ...!";
                    } else if (oBindingObj.email && oEmpData[0].managerEmail.toLowerCase() === oBindingObj.email.toLowerCase()) {
                        sText = "Manager ID and Assignee ID must be defferent...!";
                    } else {
                        sText = "There is some glitch in the code...!"
                    }
            ...
            onBaseLocValueHelpRequest:function(oEvent){
                ...
                if (!this._oGenricValueHelpDialog) {
                    this._oGenricValueHelpDialog = sap.ui.xmlfragment("com.mindset.ui.assetwisepg.workflow.fragments.Dialogs.Valuehelp", this);
            ...
            handleValueHelp: function (oEvent) {                
                ...
                let oModel = oController.getOwnerComponent().getModel("GlobalModel");
                let oSrc = oEvent.getSource();
                oController.sMatTypeSelectedItemPath = oEvent.getSource().getBindingContext("GlobalModel").getPath() || null;

                if (!this._pValueHelpDialogAsset) {
                    this._pValueHelpDialogAsset = Fragment.load({
                        id: oView.getId(),
                        name: "com.mindset.ui.assetwisepg.workflow.fragments.Dialogs.MatTypeValueHelp",
                        controller: this
                ...
                this._pValueHelpDialogAsset.then(function (oDialog1) {
                    oDialog1.open();
                });
            },
            onCloseMatTypeDialog: function (oEvent) {
                this._pValueHelpDialogAsset.then(function (oDialog1) {
                    oDialog1.close();
                });
            },
            ...
            onComboBoxChange: function (oEvent) {
                ...
                var filterString = "matType eq '" + selectedKey + "'";  // Customize as needed
                
                // Optionally, rebind the table if needed
                var oTable = this.byId("idProductsTableMatType");
                var oBinding = oTable.getBinding("items"); // This forces the table to rebind with the updated filter
                var aFilters = [];
                if (selectedKey) {
                    oGlobalModel.setProperty("/AssetMasterMatTypeTable", oGlobalModel.getProperty("/AssetMasterMatType"));
                    aFilters.push(new sap.ui.model.Filter("matType", sap.ui.model.FilterOperator.EQ, selectedKey));
                } else {
            ...
            onMatTableRowPress: function(oEvent){
                if (oEvent && oController.sMatTypeSelectedItemPath) {
                    let oSrc = oEvent.getSource();
                    let oBj = oSrc.getBindingContext("GlobalModel").getObject();
                    let oObject = oGlobalModel.getProperty(oController.sMatTypeSelectedItemPath);
                        oObject.assetID = oBj.matType;
                        oObject.assetName = oBj.name;
                        oObject.serialNo = oBj.serialNo;
                        oObject.barcode = oBj.barcodeNo;
                        oObject.assetDesc1 = oBj.desc1;
                        oObject.assetDesc2 = oBj.desc2;
                        oObject.assetDesc3 = oBj.desc3;
                    oGlobalModel.setProperty(oController.sMatTypeSelectedItemPath, oObject);                    
                    oController.onCloseMatTypeDialog();
                } else {
                    oController.onCloseMatTypeDialog();
                    MessageToast.show("Path Error...!");
                }             
            }
        });
    });
