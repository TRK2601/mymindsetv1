...
const inputValidation = async(req) => {
  ...
  // Add by Ravi Krishna Thota
  const { nextRenewalDate, previousRenewalDate, returnDate, handedOverDate, assetScrappedValue, Owner, ownedTeam, userTeam, user, barcodeNo, serialNo } = req.data;
  if (nextRenewalDate && previousRenewalDate ) {
    if (new Date(nextRenewalDate) < new Date(previousRenewalDate)) {
        req.error(400, `In Insurance Section, Next renewal date cannot be before previous renewal date.`);
    }
  }
  if (returnDate && handedOverDate ) {
    if (new Date(returnDate) < new Date(handedOverDate)) {
        req.error(400, `In Owner Section, Return date cannot be before handed over date.`);
    }
  } 
  if (assetScrappedValue) {
    const regex = /^[0-9]*\.?[0-9]+$/;
    let sresult = regex.test(assetScrappedValue);
     if (!sresult) {
      req.error(400, `In Scrapping Section, The Asset Scrapped Value must be a valid number (e.g., positive integers or decimals).`);
     } 
  }
  if (!user && userTeam) {
    req.error(400, `In Owner Section, Please add User...!`);
  } 
  if (!Owner && ownedTeam) {
    req.error(400, `In Owner Section, Please add Owner...!`);
  }
  if (barcodeNo || serialNo) {
    const regex = /^\d+$/;    
    if (serialNo) {
      let sresultSN = regex.test(serialNo);
      if (!sresultSN) {
        req.error(400, `In Basic Data Section, The Asset Serial No Value must be a valid number (e.g., positive integers).`);
       }
    }     
    if (barcodeNo) {
      let sresultBC = regex.test(barcodeNo);
      if (!sresultBC) {
        req.error(400, `In Basic Data Section, The Asset BarCode No Value must be a valid number (e.g., positive integers).`);
      } else {
        // Query to check if barcodeNo already exists
        const existing = await SELECT.one.from(AssetMaster).where({ barcodeNo });      
        if (existing) {
          if (reqID && reqID !== existing.ID) {
            req.error(400, `In Basic Data Section, the Barcode No '${barcodeNo}' is already in use.`);
          }          
        }
      }
    }
...
const assetIDFactoryFunc = (req) => {
  ...
  let newBarcode = barcode.length < 12 ? '0'.repeat(12 - barcode.length) + barcode : barcode;
...
async function getAPRequestbyEmailID(req){
  ...
  let srv = await cds.connect.to('assetwise_pg'),
      // sSrv = srv.tx(), 
